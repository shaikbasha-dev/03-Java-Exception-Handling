...
* `method2()` throws `ArithmeticException` because it attempts to divide by zero using the assignment operator (`=`).
...
* For checked exceptions, the method must explicitly either catch them or declare them with the `throws` keyword to propagate up.
